...
def prepare_3d_data(df):
    ...
    # 2. Obter o tempo atual exato em formato Julian Date
    ...
    # 3. Calcular a posição real inicial para cada satélite
...
def build_globe_figure(df_filtered, orbit_row=None, current_time_str=""):
    x_e, y_e, z_e             = _EARTH_SURFACE
    coast_x, coast_y, coast_z = _COASTLINES

    max_range = 50000 # Escala fixa para evitar zoom indesejado!
    ...
    for obj_type, color in COLOR_MAP.items():
        ...
        fig.add_trace(go.Scatter3d(
            ...
            marker=dict(size=2, color=color, opacity=0.75, line=dict(
                width=4, color='rgba(255, 255, 255, 0)'
            )),
    ...
    if orbit_row is not None:
        try:
            ...
            ox, oy, oz = np.array(ox), np.array(oy), np.array(oz)
            ...
            fig.add_trace(go.Scatter3d(
                x=[float(orbit_row['X'])], y=[float(orbit_row['Y'])], z=[float(orbit_row['Z'])],
                mode='markers',
                marker=dict(size=6, color='white', symbol='diamond', line=dict(color='yellow', width=2)),
                name='Seleccionado',
    ...
    fig.update_layout(
        ...
        # --- NOVO: A ANOTAÇÃO DO RELÓGIO ---
        annotations=[
            dict(
                text=f"🕒 {current_time_str} UTC",
                x=0.02, y=0.02, # Posição: Canto inferior esquerdo
...
fig_line = go.Figure(data=[go.Scatter(
    x=launches['EPOCH_YEAR'], y=launches['count'], mode='lines+markers', line=dict(color='#4a6fa5', width=2), marker=dict(size=4)
)])
...
def make_filter_section(group):
    ...
    return html.Div([
        ...
        ], style={'display': 'flex', 'justifyContent': 'space-between', 'alignItems': 'center', 'cursor': 'pointer', 'padding': '8px 4px', 'borderBottom': '1px solid #2d3748', 'userSelect': 'none'}),
...
<html>
    <head>
        ...
        <style>
            ...
            #conjunction-days-slider .rc-slider-handle:hover, #conjunction-days-slider .rc-slider-handle:active { border-color: #00d4ff !important; box-shadow: 0 0 8px rgba(0, 212, 255, 0.5) !important; }
...
app.layout = html.Div(style={
    ...
    html.Div(style={
        ...
        # Linha 3 - Globo e Info Flutuante
        html.Div(id='globe-container', style={
            **card_style, 'gridColumn': '1 / 4', 'gridRow': '3',
            ...
            html.Div(id='selected-info', style={
                ...
                'zIndex': '10', 'minWidth': '250px',
                'display': 'flex', 'flexDirection': 'column'
            }, children=[
                # 1. Aqui está o contentor que faltava!
                html.Div(id='satellite-data-container', children=[
                    html.Div('Clica num objecto no globo para ver a sua órbita', style={'color': '#9ca3af', 'fontSize': '13px', 'fontStyle': 'italic'})
                ]),
                # 2. E aqui está o botão fixo escondido
        ...
        html.Div(style={
            ...
            *[make_filter_section(g) for g in filter_groups],
            html.Button('Apply Filters', id='apply-filters', n_clicks=0, style={**button_style, 'marginTop': 'auto', 'width': '100%', 'backgroundColor': '#4a6fa5', 'fontWeight': 'bold', 'padding': '12px'})
        ]),
    ...
    html.Div(id='conjunction-modal', style={
        ...
        'backgroundColor': 'rgba(0,0,0,0.85)', 'zIndex': '9999', 'justifyContent': 'center', 'alignItems': 'center'
    }, children=[
        html.Div(style={
            **card_style, 'width': '800px', 'maxHeight': '85vh', 'position': 'relative', 'padding': '25px',
            'justifyContent': 'flex-start', 'alignItems': 'stretch', 'border': '1px solid #2d3748',
        }, children=[
            html.Button('✖', id='close-modal-btn', n_clicks=0, style={'position': 'absolute', 'top': '15px', 'right': '15px', 'background': 'none', 'border': 'none', 'color': '#9ca3af', 'fontSize': '18px', 'cursor': 'pointer'}),
            html.H3(id='modal-title', children='Análise de Conjunções', style={'color': '#00d4ff', 'marginTop': '0', 'marginBottom': '20px', 'fontSize': '18px', 'fontWeight': 'bold', 'letterSpacing': '0.5px'}),
            # Slider
            html.Div(style={'width': '100%', 'marginBottom': '28px'}, children=[
                ...
                dcc.Slider(
                    ...
                    tooltip={'always_visible': False, 'placement': 'bottom'}, # <--- GARANTE que a caixa branca e os tooltips desaparecem
                ),
            ]),
            ...
            dcc.Loading(color='#00d4ff', type='circle', children=[
                dash_table.DataTable(
                    ...
                    data=[], 
                    # 1. APAGA A LINHA DO page_action='none'
                    # 2. ADICIONA ISTO (Força os 10 resultados numa página única, escondendo os botões):
                    page_size=10, 
                    style_table={
                    ...
                    style_data_conditional=[
                        ...
                        {'if': {'filter_query': '{MIN_DIST_KM} < 10'}, 'backgroundColor': 'rgba(255,107,53,0.20)', 'color': '#ff6b35', 'fontWeight': 'bold'},
                        {'if': {'filter_query': '{MIN_DIST_KM} >= 10 && {MIN_DIST_KM} < 100'}, 'backgroundColor': 'rgba(255,215,0,0.08)', 'color': '#ffd700'},
                        {'if': {'state': 'active'}, 'backgroundColor': '#2d3748', 'border': '1px solid #4a6fa5'},
                    ],
...
def store_click(click_data):
    ...
    except Exception as ex: print(f"store_click error: {ex}")
    return None, None

@app.callback(
    Output('globe-3d', 'figure'),
    Output('satellite-data-container', 'children'), # <--- 1. Agora atualiza só os dados
    Output('check-conjunctions-btn', 'style'),      # <--- 2. E controla a visibilidade do botão
...
def update_globe(n_intervals, apply_clicks, selected_idx, close_clicks,
    ...
    # ESTADO INICIAL (Sem satélite clicado)
    btn_style = {'display': 'none'}
    info_children = [html.Div('Clica num objecto no globo para ver a sua órbita', style={'color': '#9ca3af', 'fontSize': '13px', 'fontStyle': 'italic'})]
    ...
    if selected_idx is not None and selected_idx in df_3d.index:
        ...
        # CONSTROÍMOS APENAS OS DADOS DA TABELA (SEM O BOTÃO AQUI DENTRO)
        info_children = [
            html.Div(orbit_row['NAME'], style={'color': 'white', 'fontSize': '15px', 'fontWeight': 'bold', 'maxWidth': '220px', 'whiteSpace': 'nowrap', 'overflow': 'hidden', 'textOverflow': 'ellipsis', 'borderBottom': '1px solid #2d3748', 'paddingBottom': '10px', 'marginBottom': '12px'}),
        ...
        # COMO HÁ UM SATÉLITE SELECIONADO, ATIVAMOS O DESIGN DO BOTÃO E TORNAMO-LO VISÍVEL
        btn_style = {**button_style, 'backgroundColor': '#4a6fa5', 'padding': '8px 10px', 'fontSize': '12px', 'fontWeight': 'bold', 'display': 'block', 'marginTop': '15px', 'width': '100%'}

    # DEVOLVEMOS 3 COISAS: O Globo, os Dados, e a Visibilidade do Botão!
    return build_globe_figure(df_f, orbit_row=orbit_row, current_time_str=current_time_str), info_children, btn_style
...
def toggle_modal(open_clicks, close_clicks, selected_idx, current_style):
    ...
    if trigger == 'close-modal-btn': return {**current_style, 'display': 'none'}, dash.no_update
...
def update_conjunction_table(open_clicks, days, norad_id):
    if not open_clicks or open_clicks < 1: return dash.no_update
    ...
    try:
        ...
        if df_result.empty:
            return [{'NAME': '✅ Órbita limpa (Sem risco detetado)', 'NORAD_ID': '-', 'MIN_DIST_KM': '-', 'TIME_UTC': '-'}]
        # -------------------------------
