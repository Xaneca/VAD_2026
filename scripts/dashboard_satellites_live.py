...
# ============================================================
# PRÉ-CÁLCULO VECTORIZADO PARA PROPAGAÇÃO LIVE
# ============================================================
# Extraímos arrays numpy uma única vez em memória para propagação rápida
_MU            = 398600.4418
_M0            = np.radians(df_3d['MEAN_ANOMALY'].values)
_MEAN_MOTION   = df_3d['MEAN_MOTION'].values * 2 * np.pi / 1440.0   # rad/min
_E             = df_3d['ECCENTRICITY'].values
_INC           = np.radians(df_3d['INCLINATION'].values)
_RAAN          = np.radians(df_3d['RA_OF_ASC_NODE'].values)
_ARGP          = np.radians(df_3d['ARG_OF_PERICENTER'].values)
_A             = ((df_3d['PERIOD'].values * 60 * np.sqrt(_MU)) / (2 * np.pi)) ** (2 / 3)

# Matrizes de rotação pré-calculadas (constantes)
_CR = np.cos(_RAAN); _SR = np.sin(_RAAN)
_CA = np.cos(_ARGP); _SA = np.sin(_ARGP)
_CI = np.cos(_INC);  _SI = np.sin(_INC)

_ROT_XX = _CR*_CA - _SR*_SA*_CI
_ROT_XY = -_CR*_SA - _SR*_CA*_CI
_ROT_YX = _SR*_CA + _CR*_SA*_CI
_ROT_YY = -_SR*_SA + _CR*_CA*_CI
_ROT_ZX = _SA*_SI
_ROT_ZY = _CA*_SI

# Grupos de object_type para indexação rápida
_TYPE_GROUPS = {}
for obj_type in ['Satellite', 'Debris', 'Rocket Body', 'Space Station']:
    idx = np.where(df_3d['OBJECT_TYPE'].values == obj_type)[0]
    _TYPE_GROUPS[obj_type] = idx

# Epoch de referência — instante dos dados TLE (usamos o epoch médio ou agora)
# Assumimos que os dados TLE representam posições no momento t0 = agora no arranque
_T0_UTC = datetime.utcnow()

def propagate_all_at_dt(dt_minutes: float):
    """
    Propaga TODOS os objectos dt_minutes minutos a partir do epoch TLE.
    Retorna dicionário {object_type: (X_array, Y_array, Z_array)}.
    Vectorizado puro — sem loops Python.
    """
    M_t = _M0 + _MEAN_MOTION * dt_minutes

    # Resolver Equação de Kepler (6 iterações de Newton-Raphson)
    E = M_t.copy()
    for _ in range(6):
        E -= (E - _E * np.sin(E) - M_t) / (1.0 - _E * np.cos(E))

    nu = 2.0 * np.arctan2(
        np.sqrt(1.0 + _E) * np.sin(E / 2.0),
        np.sqrt(1.0 - _E) * np.cos(E / 2.0),
    )

    r  = _A * (1.0 - _E**2) / (1.0 + _E * np.cos(nu))
    xo = r * np.cos(nu)
    yo = r * np.sin(nu)

    X = _ROT_XX * xo + _ROT_XY * yo
    Y = _ROT_YX * xo + _ROT_YY * yo
    Z = _ROT_ZX * xo + _ROT_ZY * yo

    result = {}
    for obj_type, idx in _TYPE_GROUPS.items():
        if len(idx) > 0:
            result[obj_type] = (X[idx], Y[idx], Z[idx])
    return result
...
# Mapeamento: object_type → índice do trace no globo
# Terra=0, Coastlines=1, Satellite=2, Debris=3, Rocket Body=4, Space Station=5
# (órbita e selected adicionados depois se existirem)
_TRACE_IDX = {obj: i + 2 for i, obj in enumerate(COLOR_MAP.keys())}
...
def build_globe_figure(df_filtered, orbit_row=None):
    ...
    if coast_x is not None:
        fig.add_trace(go.Scatter3d(
            ...
            mode='lines', line=dict(color='rgba(255,255,255,1)', width=1.5),
...
<html>
    <head>
        ...
        <style>
            ...
            /* ── Timeline slider dentro do globo ── */
            #timeline-overlay {
                position: absolute;
                bottom: 18px;
                left: 18px;
            ...
            #timeline-overlay .tl-header {
                display: flex;
                align-items: center;
                justify-content: space-between;
                margin-bottom: 6px;
            }
            #timeline-overlay .tl-label {
                color: #9ca3af;
                font-size: 10px;
                letter-spacing: 1.2px;
                text-transform: uppercase;
            }
            #timeline-overlay .tl-live-badge {
                font-size: 10px;
                font-weight: bold;
                letter-spacing: 1px;
                padding: 2px 8px;
            ...
            #timeline-overlay .tl-live-badge.paused {
                background: rgba(255, 107, 53, 0.15);
                border-color: #ff6b35;
                color: #ff6b35;
            }
            #timeline-overlay .tl-clock {
                color: #e2e8f0;
                font-size: 13px;
            ...
            /* Override Dash slider colours for timeline */
            #timeline-slider .rc-slider-track {
                background-color: #00d4ff !important;
            }
            #timeline-slider .rc-slider-handle {
                border-color: #00d4ff !important;
            ...
            #timeline-slider .rc-slider-rail {
                background-color: #2d3748 !important;
            }
...
# Timeline: 0 = LIVE, 1..432 = offset em minutos (passo 10 min, máx 3 dias = 4320 min)
_TIMELINE_MAX   = 432   # steps de 10 min → 4320 min = 3 dias
_TIMELINE_STEP  = 10    # minutos por step

_TIMELINE_MARKS = {
    0:   {'label': 'LIVE', 'style': {'color': '#00d4ff', 'fontSize': '10px'}},
    144: {'label': '+1d',  'style': {'color': '#6b7280', 'fontSize': '10px'}},
    288: {'label': '+2d',  'style': {'color': '#6b7280', 'fontSize': '10px'}},
    432: {'label': '+3d',  'style': {'color': '#6b7280', 'fontSize': '10px'}},
}
    ...
    html.Div(style={
        ...
        html.Div(id='globe-container', style={
            **card_style, 'gridColumn': '1 / 4', 'gridRow': '3',
            ...
            # ── Overlay da timeline ──────────────────────────
            html.Div(id='timeline-overlay', className='', children=[
                html.Div(className='tl-header', children=[
                    html.Span('⏱ TIMELINE', className='tl-label'),
                    html.Span('● LIVE', id='live-badge', className='tl-live-badge'),
                ]),
                dcc.Slider(
                    id='timeline-slider',
                    className='hide-tooltip',
                    min=0, max=_TIMELINE_MAX,
                    step=1, value=0,
                    marks=_TIMELINE_MARKS,
                    updatemode='drag',
                    tooltip=None,
                ),
                html.Div(id='timeline-clock', className='tl-clock',
                         children='UTC --:--:-- | +0 min'),
            ])
        ]),
...
# ============================================================
# CALLBACK — Relógio + badge LIVE/PAUSED (leve, 1 s)
# ============================================================
@app.callback(
...
def update_clock(_, slider_val):
    now_utc   = datetime.utcnow()
    offset_min = (slider_val or 0) * _TIMELINE_STEP
    display_dt = now_utc + timedelta(minutes=offset_min)

    if slider_val == 0:
        clock_str = f"UTC  {now_utc.strftime('%Y-%m-%d  %H:%M:%S')}"
        badge_txt = '● LIVE'
        badge_cls = 'tl-live-badge'
    else:
        clock_str = (
            f"UTC  {display_dt.strftime('%Y-%m-%d  %H:%M')}  "
            f"(+{offset_min // 60}h {offset_min % 60}m)"
        )
        badge_txt = '⏸ SIM'
        badge_cls = 'tl-live-badge paused'

    return clock_str, badge_txt, badge_cls

# ============================================================
# CALLBACK — Actualizar globo + posições satélites (live + filtros)
# ============================================================
@app.callback(
...
def update_globe(n_clicks, selected_idx, n_intervals, slider_val,
    ...
    # Modo LIVE: offset = minutos decorridos desde _T0_UTC
    if slider_val == 0:
        now_utc    = datetime.utcnow()
        offset_min = (now_utc - _T0_UTC).total_seconds() / 60.0

    # ------------------------------------------------------------------
    # Se apenas o interval disparou (sem mudança de filtros/selecção)
    # → usar Patch() para só actualizar x/y/z de cada trace de satélite
    # ------------------------------------------------------------------
    live_only = (
        triggered_ids == {'live-interval'} or
        (triggered_ids == {'live-interval', 'timeline-slider'} and slider_val == 0)
    )

    if live_only and selected_idx is None:
        positions = propagate_all_at_dt(offset_min)
        patched   = Patch()
        for i, obj_type in enumerate(COLOR_MAP.keys()):
            trace_idx = i + 2   # Terra=0, Coastlines=1, depois os 4 tipos
            if obj_type in positions:
                X, Y, Z = positions[obj_type]
                patched['data'][trace_idx]['x'] = X.tolist()
                patched['data'][trace_idx]['y'] = Y.tolist()
                patched['data'][trace_idx]['z'] = Z.tolist()
        # Info bar mantém-se
        info_placeholder = [
            html.Div('Clica num objecto no globo para ver a sua órbita',
                     style={'color': '#6b7280', 'fontSize': '13px', 'fontStyle': 'italic'})
        ]
        return patched, dash.no_update

    # ------------------------------------------------------------------
    # Reconstrução completa (filtros alterados, objecto seleccionado, etc.)
    # ------------------------------------------------------------------

    # Propagar todos para o offset actual
    positions = propagate_all_at_dt(offset_min)

    # Aplicar posições propagadas ao df de trabalho
    df_f = df_3d.copy()
    for obj_type, idx in _TYPE_GROUPS.items():
        if obj_type in positions:
            X, Y, Z = positions[obj_type]
    ...
    # Recalcular altitude propagada
    df_f['ALTITUDE'] = (np.sqrt(df_f['X']**2 + df_f['Y']**2 + df_f['Z']**2) - 6371).round(0)
    ...
    if selected_idx is not None and selected_idx in df_3d.index:
        orbit_row = df_3d.loc[selected_idx].copy()
        # Actualizar posição do objecto seleccionado com a propagação
        obj_type_sel = orbit_row['OBJECT_TYPE']
        local_pos    = propagate_all_at_dt(offset_min)
        if obj_type_sel in local_pos:
            type_idx_arr = _TYPE_GROUPS[obj_type_sel]
            # Posição do idx dentro do grupo
            pos_in_group = np.where(type_idx_arr == selected_idx)[0]
            if len(pos_in_group) > 0:
                g_i = pos_in_group[0]
                orbit_row = orbit_row.copy()
                orbit_row['X'] = local_pos[obj_type_sel][0][g_i]
                orbit_row['Y'] = local_pos[obj_type_sel][1][g_i]
                orbit_row['Z'] = local_pos[obj_type_sel][2][g_i]
                orbit_row['ALTITUDE'] = (
                    np.sqrt(orbit_row['X']**2 + orbit_row['Y']**2 + orbit_row['Z']**2) - 6371
                )
